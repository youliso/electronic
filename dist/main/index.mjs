import fs$k, { statSync, writeFileSync as writeFileSync$1, appendFileSync, accessSync, constants as constants$2 } from 'fs';
import require$$1, { sep as sep$1, join, resolve, extname, normalize as normalize$1 } from 'path';
...
const pathFile = path$k.join(__dirname, 'path.txt');

function getElectronPath () {
  let executablePath;
  if (fs$j.existsSync(pathFile)) {
    executablePath = fs$j.readFileSync(pathFile, 'utf-8');
  }
  if (process.env.ELECTRON_OVERRIDE_DIST_PATH) {
    return path$k.join(process.env.ELECTRON_OVERRIDE_DIST_PATH, executablePath || 'electron');
  }
  if (executablePath) {
    return path$k.join(__dirname, 'dist', executablePath);
  } else {
...
function logInfo(...val) {
    const date = new Date();
    const path = logFile +
        `${sep$1}${date.getFullYear()}-${(date.getMonth() + 1).toString().padStart(2, '0')}-${date
            .getDate()
            .toString()
    ...
    write(path, `[${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}] [info] ${data}${EOL}`);
}
/**
 * info错误
 * @param val
...
function logError(...val) {
    const date = new Date();
    const path = logFile + `${sep$1}${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}.error.log`;
    let data = '';
    val.forEach((e) => {
        try {
            if (typeof e === 'object')
    ...
    write(path, `[${date.getHours()}:${date.getMinutes()}:${date.getSeconds()}] [error] ${data}${EOL}`);
}
 ...
 * @param suffix
 */
function fileBySuffix(path, suffix) {
    try {
        let files = [];
        let dirArray = fs$k.readdirSync(path);
        for (let d of dirArray) {
            let filePath = resolve(path, d);
            let stat = fs$k.statSync(filePath);
            if (stat.isDirectory()) {
...
function delDir(path) {
    let files = [];
    if (fs$k.existsSync(path)) {
        files = fs$k.readdirSync(path);
        files.forEach((file) => {
            let curPath = path + '/' + file;
            if (fs$k.statSync(curPath).isDirectory()) {
                delDir(curPath); //递归删除文件夹
            }
            else {
                fs$k.unlinkSync(curPath); //删除文件
            }
        });
        fs$k.rmdirSync(path);
    }
...
function unlink(path) {
    return new Promise((resolve) => fs$k.unlink(path, (err) => {
        if (err)
            resolve(0);
        else
            resolve(1);
...
function access(path) {
    return new Promise((resolve) => fs$k.access(path, fs$k.constants.F_OK, (err) => {
        if (err)
 ...
 * @return 0 失败 1 成功
 */
function rename$2(path, newPath) {
    return new Promise((resolve) => {
        fs$k.rename(path, newPath, (err) => {
            if (err)
                resolve(0);
            else
                resolve(1);
...
function readFile$1(path, options) {
    return new Promise((resolve) => fs$k.readFile(path, options, (err, data) => {
        if (err)
            resolve(0);
        resolve(data);
    }));
...
function readLine(path, index) {
    const io = createInterface({
        input: fs$k.createReadStream(path)
    });
    return new Promise((resolve) => {
        switch (index) {
            case -1:
 ...
 * @returns 0 失败 1成功
 */
async function mkdir$4(path, options) {
    return new Promise((resolve) => {
        fs$k.mkdir(path, options || { recursive: true }, (err) => {
            if (err) {
                resolve(0);
            }
            resolve(1);
 ...
 * @return 0 失败 1 成功
 */
async function writeFile$1(path, data, options) {
    return new Promise((resolve) => fs$k.writeFile(path, data, options || {}, (err) => {
        if (err) {
            resolve(0);
        }
        resolve(1);
...
async function appendFile(path, data, options) {
    return new Promise((resolve) => fs$k.appendFile(path, data, options || {}, (err) => {
        if (err) {
            resolve(0);
        }
        resolve(1);
...
function fileOn() {
    ...
    electron.ipcMain.handle('file-mkdir', async (event, args) => mkdir$4(args.path, args.options));
    ...
    electron.ipcMain.handle('file-rename', async (event, args) => rename$2(args.path, args.newPath));
    electron.ipcMain.handle('file-readfile', async (event, args) => readFile$1(args.path, args.options));
    electron.ipcMain.handle('file-readline', async (event, args) => readLine(args.path, args.index));
    electron.ipcMain.handle('file-writefile', async (event, args) => writeFile$1(args.path, args.data, args.options));
...
class Global {
    ...
    async use(conf) {
        if (Array.isArray(conf)) {
            for (let index = 0; index < conf.length; index++) {
                const c = conf[index];
                try {
                    const cfg = (await readFile$1(c.path, c.opt || { encoding: 'utf-8' }));
                    if (cfg)
            ...
            try {
                const cfg = (await readFile$1(conf.path, conf.opt || { encoding: 'utf-8' }));
                if (cfg)
     ...
     * */
    getResourcesPath(type, path = './') {
        try {
            switch (type) {
                case 'platform':
                    path = normalize$1(electron.app.isPackaged
                        ? resolve(join(__dirname, '..', '..', '..', 'platform', process.platform, path))
                        : resolve(join('resources', 'platform', process.platform, path)));
                    break;
                case 'inside':
                    return (path = normalize$1(electron.app.isPackaged
            ...
            accessSync(path, constants$2.R_OK);
            return path;
        }
        catch (e) {
...
function sep() {
    return require$$1.sep;
}
function isAbsolute(str) {
    return require$$1.isAbsolute(str);
}
function dirname(str) {
    return require$$1.dirname(str);
}
function normalize(str) {
    return require$$1.normalize(str);
}
function basename(str) {
    return require$$1.basename(str);
}
function pathOn() {
...
const debug$5 = debug_1$1.default("electron-builder");
...
class HttpExecutor {
    ...
    doApiRequest(options, cancellationToken, requestProcessor, redirectCount = 0) {
        if (debug$5.enabled) {
            debug$5(`Request: ${safeStringifyJson(options)}`);
        }
    ...
    handleResponse(response, options, cancellationToken, resolve, reject, redirectCount, requestProcessor) {
        var _a;
        if (debug$5.enabled) {
            debug$5(`Response: ${response.statusCode} ${response.statusMessage}, request options: ${safeStringifyJson(options)}`);
        }
...
class DigestTransform extends stream_1$2.Transform {
    constructor(expected, algorithm = "sha512", encoding = "base64") {
        ...
        this.digester = crypto_1$4.createHash(algorithm);
    }
...
function configurePipes(options, response) {
    ...
    const fileOut = fs_1$5.createWriteStream(options.destination);
...
const randomHost = crypto_1$3.randomBytes(16);
...
function uuidNamed(name, hashMethod, version, namespace, encoding = UuidEncoding.ASCII) {
    const hash = crypto_1$3.createHash(hashMethod);
...
(function (sax) { // wrapper for non-node envs
  ...
  SAXStream.prototype.write = function (data) {
    if (typeof Buffer === 'function' &&
      ...
      if (!this._decoder) {
        var SD = require$$0$4.StringDecoder;
...
function moveSync (src, dest, opts) {
  ...
  if (!isParentRoot(dest)) mkdirpSync(path$7.dirname(dest));
...
class DownloadedUpdateHelper {
    ...
    get cacheDirForPendingUpdate() {
        return path$6.join(this.cacheDir, "pending");
    }
    ...
    async getValidCachedUpdateFile(fileInfo, logger) {
        ...
        const updateFile = path$6.join(this.cacheDirForPendingUpdate, cachedInfo.fileName);
    ...
    getUpdateInfoFile() {
        return path$6.join(this.cacheDirForPendingUpdate, "update-info.json");
    }
}
DownloadedUpdateHelper$1.DownloadedUpdateHelper = DownloadedUpdateHelper;
function hashFile(file, algorithm = "sha512", encoding = "base64", options) {
    return new Promise((resolve, reject) => {
        const hash = crypto_1$2.createHash(algorithm);
        hash.on("error", reject).setEncoding(encoding);
        fs_1$4.createReadStream(file, { ...options, highWaterMark: 1024 * 1024 /* better to use more memory but hash faster */ })
            .on("error", reject)
            .on("end", () => {
            hash.end();
            resolve(hash.read());
...
async function createTempUpdateFile(name, cacheDir, log) {
    ...
    let nameCounter = 0;
    let result = path$6.join(cacheDir, name);
    for (let i = 0; i < 3; i++) {
        try {
            ...
            result = path$6.join(cacheDir, `${nameCounter++}-${name}`);
        }
    }
    return result;
}
...
function getAppCacheDir() {
    ...
    let result;
    if (process.platform === "win32") {
        result = process.env["LOCALAPPDATA"] || path$5.join(homedir, "AppData", "Local");
    }
    else if (process.platform === "darwin") {
        result = path$5.join(homedir, "Library", "Application Support", "Caches");
    }
    else {
        result = process.env["XDG_CACHE_HOME"] || path$5.join(homedir, ".cache");
    }
    return result;
}
...
class ElectronAppAdapter {
    ...
    get appUpdateConfigPath() {
        return this.isPackaged ? path$4.join(process.resourcesPath, "app-update.yml") : path$4.join(this.app.getAppPath(), "dev-app-update.yml");
    }
    get userDataPath() {
...
function resolveFiles(updateInfo, baseUrl, pathTransformer = (p) => p) {
    ...
    const result = files.map(fileInfo => {
        ...
        return {
            url: util_1$6.newUrlFromBase(pathTransformer(fileInfo.url), baseUrl),
            info: fileInfo,
        };
    });
    ...
    if (packageInfo != null) {
        result[0].packageInfo = {
            ...packageInfo,
            path: util_1$6.newUrlFromBase(pathTransformer(packageInfo.path), baseUrl).href,
        };
    }
    return result;
}
...
class GenericProvider extends Provider_1$7.Provider {
    constructor(configuration, updater, runtimeOptions) {
        ...
        this.baseUrl = util_1$5.newBaseUrl(this.configuration.url);
    }
    get channel() {
    ...
    async getLatestVersion() {
        const channelFile = util_1$5.getChannelFilename(this.channel);
        const channelUrl = util_1$5.newUrlFromBase(channelFile, this.baseUrl, this.updater.isAddNoCacheQuery);
        for (let attemptNumber = 0;; attemptNumber++) {
            try {
...
class BitbucketProvider extends Provider_1$6.Provider {
    constructor(configuration, updater, runtimeOptions) {
        super({
            ...runtimeOptions,
        ...
        this.baseUrl = util_1$4.newBaseUrl(`https://api.bitbucket.org/2.0/repositories/${owner}/${slug}/downloads`);
    }
    get channel() {
    ...
    async getLatestVersion() {
        ...
        const channelFile = util_1$4.getChannelFilename(this.getCustomChannelName(this.channel));
        ...
        try {
            ...
            return Provider_1$6.parseUpdateInfo(updateInfo, channelFile, channelUrl);
        }
        catch (e) {
            throw builder_util_runtime_1$c.newError(`Unable to find latest version on ${this.toString()}, please ensure release exists: ${e.stack || e.message}`, "ERR_UPDATER_LATEST_VERSION_NOT_FOUND");
        }
    }
    resolveFiles(updateInfo) {
...
class BaseGitHubProvider extends Provider_1$5.Provider {
    constructor(options, defaultHost, runtimeOptions) {
        ...
        this.options = options;
        this.baseUrl = util_1$3.newBaseUrl(builder_util_runtime_1$b.githubUrl(options, defaultHost));
        const apiHost = defaultHost === "github.com" ? "api.github.com" : defaultHost;
        this.baseApiUrl = util_1$3.newBaseUrl(builder_util_runtime_1$b.githubUrl(options, apiHost));
    }
...
class GitHubProvider extends BaseGitHubProvider {
    ...
    async getLatestVersion() {
        ...
        const feedXml = (await this.httpRequest(util_1$3.newUrlFromBase(`${this.basePath}.atom`, this.baseUrl), {
        ...
        const fetchData = async (channelName) => {
            channelFile = util_1$3.getChannelFilename(channelName);
            channelFileUrl = util_1$3.newUrlFromBase(this.getBaseDownloadPath(String(tag), channelFile), this.baseUrl);
    ...
    async getLatestTagName(cancellationToken) {
        ...
        const url = options.host == null || options.host === "github.com"
            ? util_1$3.newUrlFromBase(`${this.basePath}/latest`, this.baseUrl)
            : new url_1$3.URL(`${this.computeGithubBasePath(`/repos/${options.owner}/${options.repo}/releases`)}/latest`, this.baseApiUrl);
        try {
...
class KeygenProvider extends Provider_1$4.Provider {
    constructor(configuration, updater, runtimeOptions) {
        ...
        this.configuration = configuration;
        this.updater = updater;
        this.baseUrl = util_1$2.newBaseUrl(`https://api.keygen.sh/v1/accounts/${this.configuration.account}/artifacts`);
    }
    get channel() {
    ...
    async getLatestVersion() {
        ...
        const channelFile = util_1$2.getChannelFilename(this.getCustomChannelName(this.channel));
        const channelUrl = util_1$2.newUrlFromBase(channelFile, this.baseUrl, this.updater.isAddNoCacheQuery);
        try {
...
class PrivateGitHubProvider extends GitHubProvider_1$1.BaseGitHubProvider {
    ...
    async getLatestVersion() {
        ...
        const channelFile = util_1$1.getChannelFilename(this.getDefaultChannelName());
    ...
    async getLatestVersionInfo(cancellationToken) {
        ...
        const url = util_1$1.newUrlFromBase(basePath, this.baseUrl);
        try {
    ...
    resolveFiles(updateInfo) {
        return Provider_1$3.getFileList(updateInfo).map(it => {
            const name = path$3.posix.basename(it.url).replace(/ /g, "-");
...
class AppUpdater extends events_1.EventEmitter {
    ...
    onUpdateAvailable(updateInfo) {
        this._logger.info(`Found version ${updateInfo.version} (url: ${builder_util_runtime_1$7.asArray(updateInfo.files)
            .map(it => it.url)
            .join(", ")})`);
    ...
    downloadUpdate(cancellationToken = new builder_util_runtime_1$7.CancellationToken()) {
        ...
        this._logger.info(`Downloading update from ${builder_util_runtime_1$7.asArray(updateInfoAndProvider.info.files)
            .map(it => it.url)
            .join(", ")}`);
    ...
    async loadUpdateConfig() {
        ...
        return js_yaml_1.load(await fs_extra_1$5.readFile(this._appUpdateConfigPath, "utf-8"));
    }
    ...
    async getOrCreateStagingUserId() {
        const file = path$2.join(this.app.userDataPath, ".updaterId");
        try {
            const id = await fs_extra_1$5.readFile(file, "utf-8");
            if (builder_util_runtime_1$7.UUID.check(id)) {
                return id;
            }
            else {
        ...
        const id = builder_util_runtime_1$7.UUID.v5(crypto_1$1.randomBytes(4096), builder_util_runtime_1$7.UUID.OID);
        this._logger.info(`Generated new staging user ID: ${id}`);
        try {
            await fs_extra_1$5.outputFile(file, id);
        }
        catch (e) {
    ...
    async getOrCreateDownloadHelper() {
        ...
        if (result == null) {
            ...
            const cacheDir = path$2.join(this.app.baseCachePath, dirName || this.app.name);
    ...
    async executeDownload(taskOptions) {
        ...
        function getCacheUpdateFileName() {
            ...
            if (urlPath.endsWith(`.${taskOptions.fileExtension}`)) {
                return path$2.basename(urlPath);
            }
            else {
        ...
        await fs_extra_1$5.mkdir(cacheDir, { recursive: true });
        const updateFileName = getCacheUpdateFileName();
        let updateFile = path$2.join(cacheDir, updateFileName);
        const packageFile = packageInfo == null ? null : path$2.join(cacheDir, `package-${version}${path$2.extname(packageInfo.path) || ".7z"}`);
        ...
        const removeFileIfAny = async () => {
            ...
            return await fs_extra_1$5.unlink(updateFile).catch(() => {
                // ignore
            });
        };
        ...
        try {
            ...
            await fs_extra_1$5.rename(tempUpdateFile, updateFile);
        }
        catch (e) {
...
function copyData(task, out, oldFileFd, reject, resolve) {
    const readStream = fs_1$3.createReadStream("", {
        fd: oldFileFd,
        autoClose: false,
        start: task.start,
        // end is inclusive
    ...
    if (partCount <= 1) {
        ...
        const w = (index) => {
            ...
            if (task.kind === downloadPlanBuilder_1$1.OperationKind.COPY) {
                DataSplitter_1$1.copyData(task, out, options.oldFileFd, reject, () => w(index));
            }
            else {
...
class DifferentialDownloader {
    ...
    async doDownloadFile(tasks, fdList) {
        ...
        const fileOut = fs_1$2.createWriteStream(this.options.newFile, { fd: newFileFd });
        await new Promise((resolve, reject) => {
            const streams = [];
...
async function readEmbeddedBlockMapData(file) {
    const fd = await fs_extra_1$3.open(file, "r");
    try {
    ...
    doDownloadUpdate(downloadUpdateOptions) {
        ...
        return this.executeDownload({
            ...
            task: async (updateFile, downloadOptions) => {
                ...
                await fs_extra_1$2.chmod(updateFile, 0o755);
            },
        });
    }
    doInstall(options) {
        ...
        fs_1$1.unlinkSync(appImageFile);
        let destination;
        ...
        if (options.isForceRunAfter) {
            ...
            child_process_1$3.execFileSync(destination, [], { env });
        }
        return true;
    }
}
...
class MacUpdater extends AppUpdater_1.AppUpdater {
    ...
    async doDownloadUpdate(downloadUpdateOptions) {
        ...
        try {
            ...
            const result = child_process_1$2.execFileSync("uname", ["-a"], { encoding: "utf8" });
    ...
    return new Promise(resolve => {
        ...
        child_process_1$1.execFile("powershell.exe", [
            ...
            try {
                ...
                if (data.Status === 0) {
                    const subject = builder_util_runtime_1$1.parseDn(data.SignerCertificate.Subject);
                    let match = false;
                    for (const name of publisherNames) {
                        const dn = builder_util_runtime_1$1.parseDn(name);
                        if (dn.size) {
...
function handleError(logger, error, stderr) {
    ...
    try {
        child_process_1$1.execFileSync("powershell.exe", ["-NoProfile", "-NonInteractive", "-Command", "ConvertTo-Json test"], { timeout: 10 * 1000 });
    }
    catch (testError) {
...
class Update {
    constructor(options, dirname) {
        ...
        if (process.platform === 'win32')
            this.autoUpdater = new main$1.NsisUpdater(this.options);
        else if (process.platform === 'darwin')
            this.autoUpdater = new main$1.MacUpdater(this.options);
        else
            this.autoUpdater = new main$1.AppImageUpdater(this.options);
...
export { Session, Trays, Update, Window, access, appInstance, appendFile, basename, delDir, dirname, fileBySuffix, fileOn, globalInstance, isAbsolute, logError, logInfo, logOn, mkdir$4 as mkdir, normalize, pathOn, readFile$1 as readFile, readLine, rename$2 as rename, sep, shortcutInstance, unlink, windowInstance, writeFile$1 as writeFile };
